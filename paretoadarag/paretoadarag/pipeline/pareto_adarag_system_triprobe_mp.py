...
class ParetoAdaRAGSystemTriProbeMP:
    def __init__(
        self,
        *,
        light_retriever,
    ...
    def run_probe(
        ...
        *,
        ...
        for i, qa in enumerate(batch):
            ...
            docs_l, scores_l = self.light_retriever.retrieve(qa.q)
            light_retrieve_s[i] = time.perf_counter() - t0

            docs_l = _safe_list(docs_l)
            scores_l = _safe_list(scores_l)
...
def build_system_from_cfg(cfg: dict, *, judge_enabled: bool) -> ParetoAdaRAGSystemTriProbeMP:
    ...
    return ParetoAdaRAGSystemTriProbeMP(
